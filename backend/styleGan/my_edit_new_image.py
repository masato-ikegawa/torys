...
# --------- Style Mixing の実行 -----------
def style_mixing2(vec_syn, col_styles, truncation_psi, Gs):
    ...
    #all_w = vec_syn[:6]
    all_w = vec_syn[:6]
    w_dict = {seed: w for seed, w in zip(
        all_seeds, list(all_w))}  # [layer, component]
    ...
    for row_seed in row_seeds:
        for col_seed in col_seeds:
            ...
            w[col_styles] = w[col_styles] + \
                (w_dict[col_seed][col_styles]-w[col_styles])*truncation_psi
            image = Gs.components.synthesis.run(
                w[np.newaxis], **Gs_syn_kwargs)[0]
    ...
    for (row_seed, col_seed), image in image_dict.items():
        PIL.Image.fromarray(image, 'RGB').save(
            './my/stylemix_images/'+str(row_seed)+'-'+str(col_seed)+'.png')
    ...
    r, c = 2, 6
    fig, axs = plt.subplots(r, c, figsize=(
        10, 10), subplot_kw=({'xticks': (), 'yticks': ()}))
...
def my_style_mixing2(vec_syn, col_styles, truncation_psi, my_data_dir, number, Gs):
    row_seeds = [1]
    col_seeds = [2, 3, 4, 5, 6]
    col_styles = col_styles
    truncation_psi = truncation_psi
    ...
    for (row_seed, col_seed), image in image_dict.items():
        if row_seed == 1 and col_seed == number+2:
            result_path = my_data_dir+'/stylemix_images/' + \
                str(row_seed)+'-'+str(col_seed)+'.png'
            PIL.Image.fromarray(image, 'RGB').save(
                my_data_dir+'/stylemix_images/'+str(row_seed)+'-'+str(col_seed)+'.png')
    return result_path
...
def predict(number, img_path):
    ...
    dataset_tool.create_from_images(
        tfrecord_dir=TFRECORD_DIR, image_dir=ALIGNED_IMAGES_DIR, shuffle=0)

    if os.path.exists('pretrained.pkl'):
        tflib.init_tf()
        print('Loading networks from "pretrained.pkl"...')
        with open("pretrained.pkl", mode="rb") as f:
    ...
    # ()内はマルチ解像度のデータセットを作成した時の画像枚数
    vec_syn = my_project_real_images2(1, data_dir=DATAOUT_DIR, Gs=Gs)
    ...
    # ミックス！
    result_path = my_style_mixing2(
        vec_smile, [4, 5], 1.0, my_data_dir=DATAOUT_DIR, number=number, Gs=Gs)
...
def main():
    ...
    # ミックス！
    style_mixing2(vec_smile, [4, 5], 1.0, Gs=Gs)
